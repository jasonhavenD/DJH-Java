...
public class LC01 {
	...
	private int rp = 0;
	private int x[];
	private PriorityQueue<QNode> queue;

	public LC01() {

	}

	public LC01(int n, int[] w, int[] v, int w2) {
		super();
		this.n = n;
		this.w = w;
		this.v = v;
	...
	public void Q_LC_KNAPSACK() {
		...
		while (i <= n) {
			if (cw + w[i] <= W) {// 左节点
				...
				up = bound(i);
				queue.offer(new QNode(cw + w[i], cp + v[i], up, true, i + 1, qNode));
			}
			up = bound(i + 1);
			if (up > bestp) {// 右节点
				System.out.println("0");
				queue.offer(new QNode(cw, cp, up, false, i + 1, qNode));
			}
			qNode = queue.poll();// 根据up取最大的qNode
			if (i == n) {
				// 构造最优解
				for (i = n; i >= 1; i--) {
